...
size_t grid::Grid::build(
  ...
  int* vidmap = (int*)gm.add_buf(_name + " vid map ", sizeof(int) * nv, vlexi2gs.data(), sizeof(int) * nv);
  gbuf_size += sizeof(int) * nv;
  int* eidmap = (int*)gm.add_buf(_name + " eid map ", sizeof(int) * ne, elexi2gs.data(), sizeof(int) * ne);
  gbuf_size += sizeof(int) * ne;
  ...
  for (int i = 0; i < 3; i++) {
    _gbuf.U[i] = (double*)gm.add_buf(_name + " U " + std::to_string(i), sizeof(double)* nv_gs);
    gbuf_size += sizeof(double) * nv_gs;
    _gbuf.F[i] = (double*)gm.add_buf(_name + " F " + std::to_string(i), sizeof(double)* nv_gs);
    gbuf_size += sizeof(double) * nv_gs;
    _gbuf.R[i] = (double*)gm.add_buf(_name + " R " + std::to_string(i), sizeof(double)* nv_gs);
    gbuf_size += sizeof(double) * nv_gs;
    //if (_layer == 0) {
  ...
  // finest layer
  if (layer == 0) {
    _gbuf.rho_e = (float*)gm.add_buf(_name + "rho_e ", sizeof(float) * ne_gs);
    gbuf_size += sizeof(float) * ne_gs;
    _gbuf.eActiveBits = (unsigned int*)gm.add_buf(_name + "eActiveBits", sizeof(unsigned int)*ebit._bitArray.size(), ebit._bitArray.data());
    gbuf_size += sizeof(unsigned int) * ebit._bitArray.size();
    _gbuf.eActiveChunkSum = (int*)gm.add_buf(_name + "eActiveChunkSum", sizeof(int)*ebit._chunkSat.size(), ebit._chunkSat.data());
    gbuf_size += sizeof(int) * ebit._chunkSat.size();
  ...
  if (layer == 0) {
    for (int i = 0; i < 8; i++) {
      _gbuf.v2e[i] = (int*)gm.add_buf(_name + " v2e " + std::to_string(i), sizeof(int) * nv_gs, v2ehost[i], sizeof(int) * nv);
      gbuf_size += sizeof(int) * nv_gs;
    }
  }
  ...
  if (layer >= (_skiplayer ? 3 : 1)) {
    for (int i = 0; i < 27; i++) {
      if (fineGrid != nullptr) {
        _gbuf.v2vfine[i] = (int*)gm.add_buf(_name + " v2vfine " + std::to_string(i), sizeof(int) * nv_gs, v2vfinehost[i], sizeof(int) * nv);
        gbuf_size += sizeof(int) * nv_gs;
  ...
  for (int i = 0; i < 8; i++) {
    if (v2vcoarsehost[i] != nullptr) {
      _gbuf.v2vcoarse[i] = (int*)gm.add_buf(_name + " v2vcoarse " + std::to_string(i), sizeof(int) * nv_gs, v2vcoarsehost[i], sizeof(int) * nv);
      gbuf_size += sizeof(int) * nv_gs;
    }
  }
  ...
  for (int i = 0; i < 27; i++) {
    _gbuf.v2v[i] = (int*)gm.add_buf(_name + " v2v " + std::to_string(i), sizeof(int) * nv_gs, v2vhost[i], sizeof(int) * nv);
    gbuf_size += sizeof(int) * nv_gs;
  ...
  if (_skiplayer && layer == 2) {
    for (int i = 0; i < 64; i++) {
      _gbuf.v2vfinecenter[i] = (int*)gm.add_buf(_name + " v2vfinecenter " + std::to_string(i), sizeof(int) * nv_gs, v2vfinec[i], sizeof(int) * nv);
      gbuf_size += sizeof(int) * nv_gs;
  ...
  if (_layer == 0) {
    _gbuf.g_sens = (float*)gm.add_buf(_name + " g_sens ", sizeof(float) * ne_gs);
    gbuf_size += sizeof(float) * ne_gs;
  }

  // allocate bitflag buffer for vertex and element
  _gbuf.vBitflag = (int*)gm.add_buf(_name + " vbitflag ", sizeof(int) * nv_gs, vbitflags, sizeof(int) * nv);
  gbuf_size += sizeof(int) * nv_gs;
  _gbuf.eBitflag = (int*)gm.add_buf(_name + " ebitflag ", sizeof(int) * ne_gs, ebitflags, sizeof(int) * ne);
  gbuf_size += sizeof(int) * ne_gs;
  ...
  if (_layer != 0) {
    _gbuf.rxStencil = (double*)gm.add_buf(_name + " rxStencil ", sizeof(double) * nv_gs * 27 * 9);
    gbuf_size += sizeof(double) * nv_gs * 27 * 9;
...
void Grid::force2matlab(const std::string& nam) {
  v3_toMatlab(nam, _gbuf.F);
}

void Grid::displacement2matlab(const std::string& nam) {
  v3_toMatlab(nam, _gbuf.U);
}

void Grid::residual2matlab(const std::string& nam) {
  v3_toMatlab(nam, _gbuf.R);
}
